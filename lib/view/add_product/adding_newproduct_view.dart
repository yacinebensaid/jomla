...
class _AddProductPageState extends State<AddProductPage> {
  ...
  void passpricingDetails(List<Map<String, String>> _passedPricingDetails) {
    setState(() {
      _pricingDetails = _passedPricingDetails;
    });
  }
  ...
  Map? color_details;
  Map? size_details;
  void addVariations(Map<String, Map<String, dynamic>?> _passedVariation) {
    setState(() {
      _variations.add(_passedVariation);
  ...
  void updateVariations(
      Map<String, Map<String, dynamic>?> _passedVariation, int index) {
    setState(() {
      _variations[index] = _passedVariation;
      if (_variations.isNotEmpty) {
        first_variation = _variations[0];
        color_details = first_variation!['color_details'];
        size_details = first_variation!['size_details'];
  ...
  void saveProduct() async {
    ...
    if (_variations.isNotEmpty) {
      for (Map<String, Map?> vari in _variations) {
        Map? _color_details = vari['color_details'];
        Map? _size_details = vari['size_details'];
        if ((_color_details != null && _size_details == null) ||
            (_color_details != null && _size_details != null)) {
          ...
          Reference referenceImageToUpload =
              referenceDirImages.child(uniqueFileName);
          try {
            await referenceImageToUpload.putFile(_color_details['image']);
            imageUrl = await referenceImageToUpload.getDownloadURL();
            Map<String, Map?> saveVari = {
              'size_details': vari['size_details'],
              'color_details': {
                'color': _color_details['color'].toString(),
                'color_name': _color_details['color_name'],
                'image': imageUrl,
                'quantity': _color_details['quantity'],
              }
            };
        ...
        } else if (_color_details == null && _size_details != null) {
          Map<String, Map?> saveVari = {
            'size_details': vari['size_details'],
                            ...
                            Padding(
                              ...
                              child: ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  } else if (size_details != null &&
                                      color_details == null) {
                                    Map size_quantity =
                                        variation['size_details']
                                            ['size_quantity'];
                                    return GestureDetector(
                                      onTap: () {
                                        Navigator.of(context).push(
